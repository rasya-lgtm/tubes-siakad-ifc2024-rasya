...
<div class="container-fluid p-0">
    <div class="mb-4">
        <h4 class="fw-bold text-dark mb-1" style="font-family: 'Segoe UI', sans-serif;">Edit Jadwal Kuliah</h4>
        <p class="text-muted small">Perbarui data jadwal perkuliahan untuk mata kuliah <strong>{{ $dataJadwal->matakuliah->nama_matakuliah }}</strong>.</p>
    </div>

    @php
        $dataDosen = \App\Models\Dosen::all();
        $dataMatakuliah = \App\Models\Matakuliah::all();
    @endphp
                ...
                <form action="{{ route('jadwal.update', $dataJadwal->id) }}" method="POST">
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <select name="kode_matakuliah" class="form-select rounded-3 py-2 border-light-subtle shadow-sm @error('kode_matakuliah') is-invalid @enderror" required>
                                ...
                                @foreach($dataMatakuliah as $mk)
                                    <option value="{{ $mk->kode_matakuliah }}" {{ (old('kode_matakuliah') ?? $dataJadwal->kode_matakuliah) == $mk->kode_matakuliah ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="hari" class="form-select rounded-3 py-2 border-light-subtle shadow-sm @error('hari') is-invalid @enderror" required>
                                ...
                                @foreach(['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu'] as $hari)
                                    <option value="{{ $hari }}" {{ (old('hari') ?? $dataJadwal->hari) == $hari ? 'selected' : '' }}>{{ $hari }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="datetime-local" name="jam" class="form-control rounded-3 py-2 border-light-subtle shadow-sm @error('jam') is-invalid @enderror" value="{{ old('jam') ?? date('Y-m-d\TH:i', strtotime($dataJadwal->jam)) }}" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="kelas" class="form-control rounded-3 py-2 border-light-subtle shadow-sm @error('kelas') is-invalid @enderror" value="{{ old('kelas') ?? $dataJadwal->kelas }}" placeholder="Contoh: A, B, atau C" required>
                    ...
                    <div class="mt-4 pt-4 border-top d-flex flex-column flex-sm-row gap-2">
                        <button type="submit" class="btn btn-primary px-4 rounded-3 fw-bold">
                            <i class="bi bi-arrow-repeat me-2"></i> Update Jadwal
                        </button>
